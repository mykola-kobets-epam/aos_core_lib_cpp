...
RetWithError<uuid::UUID> MbedTLSCryptoProvider::CreateSHA1(const uuid::UUID& space, const String& data)
{
    constexpr auto UUIDVersion = 5;

    auto dataBuff = Array<uint8_t>(reinterpret_cast<const uint8_t*>(data.Get()), data.Size());

    StaticArray<uint8_t, cSHA1InputDataSize> buffer = space;

    auto err = buffer.Insert(buffer.end(), dataBuff.begin(), dataBuff.end());
    if (!err.IsNone()) {
        return {{}, AOS_ERROR_WRAP(err)};
    }
    ...
    result[6] = (result[6] & 0x0f) | uint8_t((UUIDVersion & 0xf) << 4);
