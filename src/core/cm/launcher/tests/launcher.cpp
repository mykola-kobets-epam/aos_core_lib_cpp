...
InstanceInfo CreateInstanceInfo(const InstanceIdent& instance, const String& runtimeID = "1.0.0",
    ...
    UpdateItemType updateType = UpdateItemTypeEnum::eService, uint32_t uid = 0, Time timestamp = {},
    bool cached = false)
{
    InstanceInfo result;
...
RunInstanceRequest CreateRunInstanceRequest(const std::string& itemID, const std::string& subjectID = "",
    uint64_t priority = 0, size_t numInstances = 1, const std::string& providerID = "",
    UpdateItemType itemType = UpdateItemTypeEnum::eService, const std::vector<std::string>& labels = {})
{
...
PartitionInfo CreatePartitionInfo(const std::string& name, const std::string& path, size_t totalSize)
{
    ...
    bool provisioned = true, NodeState state = NodeStateEnum::eOnline, Error error = ErrorEnum::eNone)
{
    ...
    nodeInfo.mRuntimes  = Array<RuntimeInfo>(runtimes.data(), runtimes.size());
    nodeInfo.mResources = Array<ResourceInfo>(resources.data(), resources.size());
...
TEST_F(CMLauncherTest, InstancesWithOutdatedTTLRemovedOnStart)
{
    ...
    ASSERT_TRUE(mStorage
            .AddInstance(CreateInstanceInfo(CreateInstanceIdent(cService1), "", "", "", UpdateItemTypeEnum::eService,
                5000, Time::Now().Add(-25 * Time::cHours), true))
            .IsNone());

    // Add instance with current timestamp.
    ASSERT_TRUE(mStorage
            .AddInstance(CreateInstanceInfo(
                CreateInstanceIdent(cService2), "", "", "", UpdateItemTypeEnum::eService, 5001, Time::Now(), true))
            .IsNone());
...
TEST_F(CMLauncherTest, CacheInstances)
{
    ...
    // Run instances 1
    StaticArray<RunInstanceRequest, cMaxNumInstances> RunInstanceRequest1;
    RunInstanceRequest1.PushBack(CreateRunInstanceRequest(cService1, cSubject1, 50, 1));
    RunInstanceRequest1.PushBack(CreateRunInstanceRequest(cService2, cSubject1, 50, 1));
    RunInstanceRequest1.PushBack(CreateRunInstanceRequest(cService3, cSubject1, 50, 1));

    ASSERT_TRUE(mLauncher.RunInstances(RunInstanceRequest1).IsNone());
    ...
    // Run instances 2
    StaticArray<RunInstanceRequest, cMaxNumInstances> RunInstanceRequest2;
    RunInstanceRequest2.PushBack(CreateRunInstanceRequest(cService1, cSubject1, 50, 1));

    ASSERT_TRUE(mLauncher.RunInstances(RunInstanceRequest2).IsNone());
...
TEST_F(CMLauncherTest, Components)
{
    ...
    // Run instances
    StaticArray<RunInstanceRequest, cMaxNumInstances> RunInstanceRequest;
    RunInstanceRequest.PushBack(
        CreateRunInstanceRequest(cComponent1, cSubject1, 50, 1, "", UpdateItemTypeEnum::eComponent));

    ASSERT_TRUE(mLauncher.RunInstances(RunInstanceRequest).IsNone());

    ASSERT_TRUE(mLauncher.Stop().IsNone());

    // Check sent run requests
    std::map<std::string, InstanceRunnerStub::NodeRunRequest> expectedRunInstanceRequests;
    InstanceRunnerStub::NodeRunRequest                        remoteRunInstanceRequest = {{},
                               {CreateAosInstanceInfo(
            CreateInstanceIdent(cComponent1, cSubject1, 0), cRootfsImageID, cRunnerRootfs, 0, "2", 50)}};
    expectedRunInstanceRequests[cNodeIDRemoteSM1]                                      = remoteRunInstanceRequest;

    EXPECT_EQ(mInstanceRunner.GetRunRequest(), expectedRunInstanceRequests);
...
struct TestData {
    ...
    StaticArray<RunInstanceRequest, cMaxNumInstances> mRunInstanceRequests;

    std::map<std::string, InstanceRunnerStub::NodeRunRequest> mExpectedRunInstanceRequests;
    ...
    bool                                                      mRebalancing;
    bool                                                      mIsStateStorageOnSamePartition = false;
};
...
TestData TestItemNodePriority()
{
    ...
    // Desired instances
    testData.mRunInstanceRequests.PushBack(CreateRunInstanceRequest(cService1, cSubject1, 100, 2));
    testData.mRunInstanceRequests.PushBack(CreateRunInstanceRequest(cService2, cSubject1, 50, 2));
    testData.mRunInstanceRequests.PushBack(CreateRunInstanceRequest(cService3, cSubject1, 0, 2));
    ...
    testData.mExpectedRunInstanceRequests[cNodeIDLocalSM].mStartInstances = localSMRequests;
    ...
    testData.mExpectedRunInstanceRequests[cNodeIDRunxSM].mStartInstances = runxSMRequests;
...
TestData TestItemLabels()
{
    ...
    // Desired instances
    testData.mRunInstanceRequests.PushBack(
        CreateRunInstanceRequest(cService1, cSubject1, 100, 2, "", UpdateItemTypeEnum::eService, {"label2"}));
    testData.mRunInstanceRequests.PushBack(
        CreateRunInstanceRequest(cService2, cSubject1, 50, 2, "", UpdateItemTypeEnum::eService, {"label1"}));
    testData.mRunInstanceRequests.PushBack(
        CreateRunInstanceRequest(cService3, cSubject1, 0, 2, "", UpdateItemTypeEnum::eService, {"label1"}));
    ...
    testData.mExpectedRunInstanceRequests[cNodeIDRemoteSM1].mStartInstances = remoteSM1Requests;
...
TestData TestItemResources()
{
    ...
    testData.mExpectedRunInstanceRequests[cNodeIDRemoteSM1].mStartInstances = remoteSMRequests;
...
TestData TestItemStorageRatio()
{
    ...
    // Desired instances
    testData.mRunInstanceRequests.PushBack(CreateRunInstanceRequest(cService1, cSubject1, 100, 5));
...
TestData TestItemStateStorageOnSamePartition()
{
    TestData testData;
    testData.mTestCaseName                  = "state storage on same partition";
    testData.mRebalancing                   = false;
    testData.mIsStateStorageOnSamePartition = true;
    ...
    // Service configs
    testData.mServiceConfigs[cService1] = CreateServiceConfig(
        {cRunnerRunc}, {}, CreateServiceQuotas(500, 500, 0, 0), CreateRequestedResources(300, 300, 0, 0));

    // Desired instances
    testData.mRunInstanceRequests.PushBack(CreateRunInstanceRequest(cService1, cSubject1, 100, 5));

    // Expected run requests - only 1 instance should fit due to state and storage limits
    std::vector<aos::InstanceInfo> localSMRequests;
    localSMRequests.push_back(
        CreateAosInstanceInfo(CreateInstanceIdent(cService1, cSubject1, 0), cImageID1, cRunnerRunc, 5000, "2", 100));
    ...
    // Expected run status - 1 succeed, і4 fails due to state and storage limits
    testData.mExpectedRunStatus.PushBack(CreateInstanceStatus({cService1, cSubject1, 0}, cNodeIDLocalSM, cRunnerRunc));
    testData.mExpectedRunStatus.PushBack(
        CreateInstanceStatus({cService1, cSubject1, 1}, "", "", InstanceStateEnum::eFailed, ErrorEnum::eNoMemory));
    testData.mExpectedRunStatus.PushBack(
        CreateInstanceStatus({cService1, cSubject1, 2}, "", "", InstanceStateEnum::eFailed, ErrorEnum::eNoMemory));
    testData.mExpectedRunStatus.PushBack(
        CreateInstanceStatus({cService1, cSubject1, 3}, "", "", InstanceStateEnum::eFailed, ErrorEnum::eNoMemory));
    testData.mExpectedRunStatus.PushBack(
        CreateInstanceStatus({cService1, cSubject1, 4}, "", "", InstanceStateEnum::eFailed, ErrorEnum::eNoMemory));
...
TestData TestItemCpuRatio()
{
    ...
    testData.mExpectedRunInstanceRequests[cNodeIDRemoteSM2].mStartInstances = remoteSM2Requests;
...
TestData TestItemRebalancing()
{
    ...
    testData.mRunInstanceRequests.PushBack(CreateRunInstanceRequest(cService1, cSubject1, 100, 1));
    testData.mRunInstanceRequests.PushBack(CreateRunInstanceRequest(cService2, cSubject1, 50, 1));
    testData.mRunInstanceRequests.PushBack(CreateRunInstanceRequest(cService3, cSubject1, 50, 1));
    ...
    testData.mExpectedRunInstanceRequests[cNodeIDLocalSM] = localSMRequest;
    ...
    testData.mExpectedRunInstanceRequests[cNodeIDRemoteSM1] = remoteSM1Request;
    ...
    testData.mExpectedRunInstanceRequests[cNodeIDRemoteSM2] = remoteSM2Request;
...
TEST_F(CMLauncherTest, Balancing)
{
    ...
    std::vector<TestData> testItems = {TestItemNodePriority(), TestItemLabels(), TestItemResources(),
        TestItemStorageRatio(), TestItemStateRatio(), TestItemCpuRatio(), TestItemRamRatio(), TestItemRebalancing(),
        TestItemRebalancingPolicy(), TestItemStateStorageOnSamePartition()};
    ...
    for (size_t i = 0; i < testItems.size(); ++i) {
        ...
        mStorageState.SetTotalStateSize(1024);
        mStorageState.SetTotalStorageSize(1024);
        mStorageState.SetIsSamePartition(testItem.mIsStateStorageOnSamePartition);
        ...
        for (const auto& nodeID : nodeIDs) {
            mResourceManager.SetNodeConfig(nodeID, cNodeTypeVM, testItem.mNodeConfigs[nodeID]);
        }
        ...
        // Run instances
        ASSERT_TRUE(mLauncher.RunInstances(testItem.mRunInstanceRequests).IsNone());
        ...
        // Check sent run requests
        EXPECT_EQ(mInstanceRunner.GetRunRequest(), testItem.mExpectedRunInstanceRequests);
    }
}
