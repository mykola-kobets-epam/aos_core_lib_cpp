...
/**
 * Stub implementation of InstanceRunnerItf interface.
 */
...
public:
    ...
    /**
     * Returns the map of nodeID to node run requests.
     */
    const std::map<std::string, NodeRunRequest>& GetRunRequest() const { return mNodeInstances; }

    /**
     * Returns node run request for a specific nodeID.
     */
