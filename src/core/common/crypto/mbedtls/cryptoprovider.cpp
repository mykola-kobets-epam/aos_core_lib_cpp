    ...
    const uint8_t** pp, int64_t& length, int32_t& tag, int32_t& xclass, bool& isConstructed, int64_t size)
{
    ...
    // Read length.
    size_t len = 0;
    if (int32_t ret = mbedtls_asn1_get_len(const_cast<uint8_t**>(&p), end, &len); ret != 0) { // NOSONAR cpp:M23_090
    ...
    const Array<uint8_t>& data, const asn1::ASN1ParseOptions& opt, int32_t& value)
{
    ...
    if (int32_t ret = mbedtls_asn1_get_int(const_cast<uint8_t**>(&p), end, &value); ret != 0) { // NOSONAR cpp:M23_090
        if (opt.mOptional) {
            return {AOS_ERROR_WRAP(ErrorEnum::eNotFound), data};
        }
    ...
    const Array<uint8_t>& data, const asn1::ASN1ParseOptions& opt, Array<uint8_t>& result)
{
    ...
    if (int32_t ret = mbedtls_asn1_get_mpi(const_cast<uint8_t**>(&p), end, &mpi); ret != 0) { // NOSONAR cpp:M23_090
        if (opt.mOptional) {
            return {AOS_ERROR_WRAP(ErrorEnum::eNotFound), data};
        }
    ...
    const Array<uint8_t>& data, const asn1::ASN1ParseOptions& opt, asn1::ObjectIdentifier& oid)
{
    ...
    int32_t ret
        = mbedtls_asn1_get_tag(const_cast<uint8_t**>(&p), end, &buf.len, MBEDTLS_ASN1_OID); // NOSONAR cpp:M23_090
    if (ret != 0) {
        if (opt.mOptional) {
            return {AOS_ERROR_WRAP(ErrorEnum::eNotFound), data};
        }
    ...
    const Array<uint8_t>& data, const asn1::ASN1ParseOptions& opt, Array<uint8_t>& result)
{
    ...
    size_t len = 0;
    if (int32_t ret = mbedtls_asn1_get_tag(const_cast<uint8_t**>(&p), end, &len, // NOSONAR cpp:M23_090
